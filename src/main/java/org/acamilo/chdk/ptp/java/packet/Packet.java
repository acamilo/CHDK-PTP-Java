...
public class Packet {
	...
	public short decodeShort(int index, ByteOrder order){
		switch(order){
		case BigEndian: return (short) ((0x000000ff&(int)packet[index+1])|(0x000000ff&(int)packet[index])<<8);
		case LittleEndian: return (short) ((0x000000ff&(int)packet[index])|(0x000000ff&(int)packet[index+1])<<8);
		}
		return 0;
	}
	public int decodeInt(int index, ByteOrder order){
		switch(order){
		case LittleEndian: return (0x000000ff&(int)packet[index])|(0x000000ff&(int)packet[index+1])<<8|(0x000000ff&(int)packet[index+2])<<16|(0x000000ff&(int)packet[index+3])<<24;
		case BigEndian: return (0x000000ff&(int)packet[index+3])|(0x000000ff&(int)packet[index+2])<<8|(0x000000ff&(int)packet[index+1])<<16|(0x000000ff&(int)packet[index])<<24;
		}
		return 0;
	}
