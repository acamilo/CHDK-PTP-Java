    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < totalInterfaces.size(); i++) {
                ...
                for (int j = 0; j < totalEndpoints.size(); j++) {
                    ...
                    if (ep.getDirection() == 0 && ep.getType() == 2) {
                        System.out
                                .println("\t\t\tAssigning Bulk OUT endpoint #" + j + " to camOut");
            ...
            if (camIn == null || camOut == null)
                System.out.println("Didn't find my endpoints Something verry bad happened..");
            else
                System.out.println("\tFound my endpoints, Building pipe");
            ...
            p = new PTPPacket(PTPPacket.PTP_USB_CONTAINER_COMMAND, PTPPacket.PTP_OPPCODE_CHDK, 1,
                    new byte[] { 12, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00 });
            session.sendPTPPacket(p);
            /*
             * p = new PTPPacket(PTPPacket.PTP_USB_CONTAINER_DATA,
             * PTPPacket.PTP_OPPCODE_CHDK, 2, "switch_mode_usb(1)\0".getBytes());
             * session.sendPTPPacket(p);
             */
