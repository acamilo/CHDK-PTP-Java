...
public class Camera {
    ...
    public Camera(short vid, short pid) throws SecurityException, UsbException,
	    CameraNotFoundException, UnsupportedEncodingException,
	    UsbDisconnectedException, CameraConnectionException {
    ...
    public Camera(String SerialNo) throws SecurityException, UsbException,
	    ...
	    CameraNotFoundException, CameraConnectionException {
	UsbServices services = UsbHostManager.getUsbServices();
	UsbHub rootHub = services.getRootUsbHub();
	...
	p.encodeInt(PTPPacket.iPTPCommandARG0, 7, ByteOrder.LittleEndian); // oppcode
									   // 7
									   // is
									   // execute
									   // script
	p.encodeInt(PTPPacket.iPTPCommandARG1, 0, ByteOrder.LittleEndian); // i'm
									   // going
									   // to
									   // hell
									   // for
	...
	// Now we prepare the script and send it as a DATA
	// chdk.ptp.java.connection.packet
	script += "\0"; // make sure string is null terminated
	p = new PTPPacket(PTPPacket.PTP_USB_CONTAINER_DATA,
		PTPPacket.PTP_OPPCODE_CHDK, 0, script.getBytes());
	...
	PTPPacket p = new PTPPacket(PTPPacket.PTP_USB_CONTAINER_COMMAND,
		PTPPacket.PTP_OPPCODE_CHDK, 0, new byte[8]); // 7,0x00,0x00,0x00,0x00,0x00,0x00,0x00}
	p.encodeInt(PTPPacket.iPTPCommandARG0, 12, ByteOrder.LittleEndian); // oppcode
									    // 12
									    // is
									    // transfer
									    // framebuffer
	p.encodeInt(PTPPacket.iPTPCommandARG1, 1, ByteOrder.LittleEndian); // 1
									   // sends
									   // viewport
	...
	for (int i = 0; i < totalInterfaces.size(); i++) {
		...
		if (ep.getDirection() == -128 && ep.getType() == 2) {
		    System.out.println("\t\t\tAssigning Bulk In endpoint #" + j
			    + " to camIn");
		    camIn = ep; // Bulk IN endpoint
		}
		if (ep.getDirection() == 0 && ep.getType() == 2) {
		    System.out.println("\t\t\tAssigning Bulk OUT endpoint #"
			    + j + " to camOut");
