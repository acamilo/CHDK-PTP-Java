...
/**
 * Contains logic required to decode a raw CHDK viewport bytes {@link Packet} into a readable {@link
 * BufferedImage}
 *
...
public class CHDKScreenImage extends Packet {
  ...
  public static final int FrameBuffer_YUV8 = 0; // UYV YYY, used for live view
  public static final int FrameBuffer_PAL8 = 1; // 8 bit palleted. used for
   ...
   *
   * @param packet containing viewport data
   */
  ...
  /**
   * Decodes a 6 byte uYvYYY 4 pixel group into a provided ARGB int array /suitable for a
   * bufferedImage [color u component for all 4][pixel 0 brightness][color v component for all
   * 4][pixel 1 brightness][pixel 2 brightness][pixel 3 brightness]
   *
  ...
  public BufferedImage decodeViewport() {
    ...
    int buflen = ((this.viewportBufferWidth() * this.viewportVisibleHeight()) / 4) * 6;

    BufferedImage b =
        new BufferedImage(
            this.viewportBufferWidth(), this.viewportVisibleHeight(), BufferedImage.TYPE_INT_RGB);
    ...
    for (int i = 0; i < buflen; i += 6) {
      ...
      convertedImageArray[rgbIndex + 0] = // Alpha
          (yuv_to_r(y0, v) << 16) & (0xff0000)
              | // Red
              (yuv_to_g(y0, v, u) << 8) & (0xff00)
              | // Green
              (yuv_to_b(y0, u)) & (0xff); // Blue
      convertedImageArray[rgbIndex + 1] = // Alpha
          (yuv_to_r(y1, v) << 16) & (0xff0000)
              | // Red
              (yuv_to_g(y1, v, u) << 8) & (0xff00)
              | // Green
              (yuv_to_b(y1, u)) & (0xff); // Blue; // Blue
      convertedImageArray[rgbIndex + 2] = // Alpha
          (yuv_to_r(y2, v) << 16) & (0xff0000)
              | // Red
              (yuv_to_g(y2, v, u) << 8) & (0xff00)
              | // Green
              (yuv_to_b(y2, u)) & (0xff); // Blue // Blue
      convertedImageArray[rgbIndex + 3] = // Alpha
          (yuv_to_r(y3, v) << 16) & (0xff0000)
              | // Red
              (yuv_to_g(y3, v, u) << 8) & (0xff00)
              | // Green
  ...
  // right.
  // cool.
  private static final int iViewDataHeaderViewportDescriptorStart =
      20; // in-chdk.ptp.java.connection.packet
  // pointer
  // to
  // viewport
  // descriptor
  // (ui
  // overlay)
  private static final int iViewDataHeaderBitmapDescriptorStart =
      24; // in-chdk.ptp.java.connection.packet-pointer
  ...
  public int getMajorVersion() {
    return this.decodeInt(iViewDataHeaderVersionNumberMajor, ByteOrder.LittleEndian);
  }

  public int getMinorVersion() {
    return this.decodeInt(iViewDataHeaderVersionNumberMinor, ByteOrder.LittleEndian);
  }

  public int getLCDAspectRatio() {
    return this.decodeInt(iViewDataHeaderLCDAspectRatio, ByteOrder.LittleEndian);
  }

  public int getPalleteType() {
    return this.decodeInt(iViewDataHeaderPalleteType, ByteOrder.LittleEndian);
  }

  public int getPaletteDataStart() {
    return this.decodeInt(iViewDataHeaderPalleteDataStart, ByteOrder.LittleEndian);
  }

  public int getViewportDescriptorStart() {
    return this.decodeInt(iViewDataHeaderViewportDescriptorStart, ByteOrder.LittleEndian);
  }

  public int getBitmapDescriptorStart() {
    return this.decodeInt(iViewDataHeaderBitmapDescriptorStart, ByteOrder.LittleEndian);
  }
  ...
  public int viewportFramebufferType() {
    return this.decodeInt(
        iDescriptorFrameBufferType + getViewportDescriptorStart(), ByteOrder.LittleEndian);
  }

  public int viewportDataStart() {
    return this.decodeInt(
        iDescriptorFrameBufferDataStart + getViewportDescriptorStart(), ByteOrder.LittleEndian);
  }
  ...
  public int viewportBufferWidth() {
    return this.decodeInt(
        iDescriptorFrameBufferWidth + getViewportDescriptorStart(), ByteOrder.LittleEndian);
  }
  ...
  public int viewportVisibleWidth() {
    return this.decodeInt(
        iDescriptorFrameBufferVisibleWidth + getViewportDescriptorStart(), ByteOrder.LittleEndian);
  }

  public int viewportVisibleHeight() {
    return this.decodeInt(
        iDescriptorFrameBufferVisibleHeight + getViewportDescriptorStart(), ByteOrder.LittleEndian);
  }
  ...
  public int viewportMarginLeft() {
    return this.decodeInt(
        iDescriptorFrameBufferMarginLeft + getViewportDescriptorStart(), ByteOrder.LittleEndian);
  }

  public int viewportMarginTop() {
    return this.decodeInt(
        iDescriptorFrameBufferMarginTop + getViewportDescriptorStart(), ByteOrder.LittleEndian);
  }

  public int viewportMarginRight() {
    return this.decodeInt(
        iDescriptorFrameBufferMarginRight + getViewportDescriptorStart(), ByteOrder.LittleEndian);
  }

  public int viewportMarginBottom() {
    return this.decodeInt(
        iDescriptorFrameBufferMarginBottom + getViewportDescriptorStart(), ByteOrder.LittleEndian);
  }

  // Bitmap Metadata
  public int bitmapFramebufferType() {
    return this.decodeInt(
        iDescriptorFrameBufferType + getBitmapDescriptorStart(), ByteOrder.LittleEndian);
  }

  public int bitmapDataStart() {
    return this.decodeInt(
        iDescriptorFrameBufferDataStart + getBitmapDescriptorStart(), ByteOrder.LittleEndian);
  }
  ...
  public int bitmapBufferWidth() {
    return this.decodeInt(
        iDescriptorFrameBufferWidth + getBitmapDescriptorStart(), ByteOrder.LittleEndian);
  }
  ...
  public int bitmapVisableWidth() {
    return this.decodeInt(
        iDescriptorFrameBufferVisibleWidth + getBitmapDescriptorStart(), ByteOrder.LittleEndian);
  }

  public int bitmapVisableHeight() {
    return this.decodeInt(
        iDescriptorFrameBufferVisibleHeight + getBitmapDescriptorStart(), ByteOrder.LittleEndian);
  }
  ...
  public int bitmapMarginLeft() {
    return this.decodeInt(
        iDescriptorFrameBufferMarginLeft + getBitmapDescriptorStart(), ByteOrder.LittleEndian);
  }

  public int bitmapMarginTop() {
    return this.decodeInt(
        iDescriptorFrameBufferMarginTop + getBitmapDescriptorStart(), ByteOrder.LittleEndian);
  }

  public int bitmapMarginRight() {
    return this.decodeInt(
        iDescriptorFrameBufferMarginRight + getBitmapDescriptorStart(), ByteOrder.LittleEndian);
  }

  public int bitmapMarginBottom() {
    return this.decodeInt(
        iDescriptorFrameBufferMarginBottom + getBitmapDescriptorStart(), ByteOrder.LittleEndian);
  }
  ...
  public byte clip_yuv(int v) {
    if (v > 255) return (byte) 0xFF; // cap on 255
    else if (v < 0) return (byte) 0x00;
    else return (byte) v;
  }

  // illustration for the inserted & 0xFF
  // byte test = (byte) 0xDE;
  // System.out.println("in fact, " + ( test << 12 ) + " is very different from " + (( test & 0xFF)
  // << 12 ) +
