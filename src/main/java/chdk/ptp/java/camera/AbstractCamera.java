...
public abstract class AbstractCamera implements ICamera {
  ...
  /**
   * USB reset required for camera initialization after close previous connection without
   * disconnect.
   */
  ...
  public int executeLuaCommand(String command)
    ...
    PTPPacket p =
        new PTPPacket(
            PTPPacket.PTP_USB_CONTAINER_COMMAND, PTPPacket.PTP_OPPCODE_CHDK, 0, new byte[8]);
    ...
    p =
        new PTPPacket(
            PTPPacket.PTP_USB_CONTAINER_DATA,
  ...
  private void waitScriptReady() throws CameraConnectionException {
    ...
    try {
      do {
        ...
        p.encodeInt(PTPPacket.iPTPCommandARG0, PTPPacket.CHDK_ScriptStatus, ByteOrder.LittleEndian);
  ...
  private Object readScriptMsg(int scriptId) throws CameraConnectionException, PTPTimeoutException {
  ...
  public byte[] getByteView() throws CameraConnectionException {
    try {
      ...
      // oppcode 12 is transfer framebuffer
      p.encodeInt(PTPPacket.iPTPCommandARG0, PTPPacket.CHDK_GetDisplayData, ByteOrder.LittleEndian);
  ...
  public byte[] getBytePicture() throws CameraConnectionException, CameraShootException {
    try {
      ...
      ready.encodeInt(
          PTPPacket.iPTPCommandARG0, PTPPacket.CHDK_RemoteCaptureIsReady, ByteOrder.LittleEndian);
      ...
      getChunk.encodeInt(
          PTPPacket.iPTPCommandARG0, PTPPacket.CHDK_RemoteCaptureGetData, ByteOrder.LittleEndian);

      getChunk.encodeInt(
          PTPPacket.iPTPCommandARG1,
          1, // image type
          ByteOrder.LittleEndian);
  ...
  public void setZoom(int zoomPosition) throws PTPTimeoutException, GenericCameraException {
    ...
    int waitTime =
        (int) ((Math.abs(zoomPosition - getZoom()) * 1.0 / getZoomSteps()) * 4000.0) + 500;
   ...
   *
   * <p>Similar to set_aflock(), this function places the camera in a mode where SD override
   * commands (e.g. set_focus) can be used and the value requested will be maintained across
   * multiple shots.
   *
   * <p>Note: works on almost all cameras, even those that do not have a Canon MF mode. More
   * information here: http://chdk.setepontos.com/index.php?topic=11078.0
   *
   * @param lock true locks the camera in MF mode and false unlocks it.
   ...
   *
   * <p>It is like Canon's built-in manual focus mode (MF), but better because focus stays locked
   * even after camera returns from deep display sleep (via display key cycle or print button
   * shortcut), which is very good for timelapse.
   *
   * <p>Note: In older versions of CHDK (prior to 1.3.0), this function does not work on all cameras
  ...
  /**
   * Returns the propset number used by the camera. Returns 1 for propset1, 2 for propset2, 3 for
   * propset3 etc. Useful for writing scripts that are not dependent on a particular camera model or
   * DIGIC processor. Information about values used for propsets can be found here:
  ...
  /**
   * get_prop (and set_prop) can be used with propcase.lua library which is included in the
   * 'complete' download packages. This module loads a table which maps camera property case names
   * the correct number for a given camera.
   *
   * <p>There are several different "property sets" known for CHDK cameras: Propset 1 on most Digic
   * II cameras, propset 2 on most Digic III and some Digic IV, and further propsets on later Digic
   * IV and Digic V. During the CHDK build process a Lua library with a table of the property case
   * names and numbers is generated for each propset. These generated tables are saved in the
   * \CHDK\LUALIB\GEN\ folder.
   */
