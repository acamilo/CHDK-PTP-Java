...
public class CHDKCamera {
	...
	public CHDKCamera(short vid, short pid) throws SecurityException, UsbException, CameraNotFoundException, UnsupportedEncodingException, UsbDisconnectedException, CameraConnectionException{
        ...
        UsbDevice dev = findDevice(rootHub,(short)vid,(short)pid);
		if (dev==null) throw new CameraNotFoundException();
		connection=getConenctionFromUSBDevice(dev);
		
	}
	public CHDKCamera(String SerialNo) throws SecurityException, UsbException, UnsupportedEncodingException, UsbDisconnectedException, CameraNotFoundException, CameraConnectionException{
        UsbServices services = UsbHostManager.getUsbServices();
        UsbHub rootHub = services.getRootUsbHub();
	...
	public boolean executeLuaScript(String script){
		...
		PTPPacket p = new PTPPacket(PTPPacket.PTP_USB_CONTAINER_COMMAND, PTPPacket.PTP_OPPCODE_CHDK, 0, new byte[8]); //7,0x00,0x00,0x00,0x00,0x00,0x00,0x00}
		p.encodeInt(PTPPacket.iPTPCommandARG0, 7, ByteOrder.LittleEndian); // oppcode 7 is execute script
		p.encodeInt(PTPPacket.iPTPCommandARG1, 0, ByteOrder.LittleEndian); // i'm going to hell for this
	...
	public BufferedImage getView() throws CameraConnectionException{
		...
		PTPPacket p = new PTPPacket(PTPPacket.PTP_USB_CONTAINER_COMMAND, PTPPacket.PTP_OPPCODE_CHDK, 0, new byte[8]); //7,0x00,0x00,0x00,0x00,0x00,0x00,0x00}
		p.encodeInt(PTPPacket.iPTPCommandARG0, 12, ByteOrder.LittleEndian); // oppcode 12 is transfer framebuffer
		p.encodeInt(PTPPacket.iPTPCommandARG1, 1, ByteOrder.LittleEndian); // 1 sends viewport
		...
		if (p.getContainerCommand()==p.PTP_USB_CONTAINER_DATA){
			...
			throw new CameraConnectionException("Camera Did not respond to a Live View request with a data packet!");
		}
		p = connection.getResponse();
		if (p.getContainerCommand()==PTPPacket.PTP_USB_CONTAINER_RESPONSE && p.getOppcode()==PTPPacket.PTP_OPPCODE_Response_OK) return image;
		else throw new CameraConnectionException("Camera Did not end session with an OK response even though a data packet was sent!");
	}
	...
	private UsbDevice findDeviceBySerialNumber(UsbHub hub,String serialNo) throws UnsupportedEncodingException, UsbDisconnectedException, UsbException {
		System.out.println("Processing Attatched devices for "+hub);
		   for (UsbDevice device : (List<UsbDevice>) hub.getAttachedUsbDevices())
	        {
	            UsbDeviceDescriptor desc = device.getUsbDeviceDescriptor();
	            ...
	            if (desc.bDeviceClass()!=0) return null; // Device we care about has class in descriptor.
	            if (device.getManufacturerString().equals(serialNo)) return device;
	            }
	...
	private PTPConnection getConenctionFromUSBDevice(UsbDevice dev) throws UnsupportedEncodingException, UsbDisconnectedException, UsbException, CameraConnectionException{
		// device info
        ...
        if (camIn==null || camOut==null) throw new CameraConnectionException("Didn't find my endpoints Something verry bad happened..");
        else System.out.println("\tFound my endpoints, Building PTPConnection");
